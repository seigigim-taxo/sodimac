...
export class CountingPageComponent implements ViewWillEnter {
   ...
   * escáner quedaría aceptando lecturas contra nada. ConteoFacade.scan() las
   * devuelve como 'rechazado' y la pantalla las pinta como FUERA DE MUESTRA —
   * o sea, el operador escaneaba, perdía la lectura y encima leía que el
   * producto no pertenecía a la muestra.
   */
    ...
    try {
      ...
      if (!this.ajustes.sincronizacionAutomatica()) {
        ...
        try {
          ...
          /*
           * sincronizar() no lanza si falla —atrapa el error para no romper la
           * lista—, así que el resultado hay que leerlo del booleano. Mirando
           * solo el catch, quedarse sin señal se anunciaba como éxito.
           */
