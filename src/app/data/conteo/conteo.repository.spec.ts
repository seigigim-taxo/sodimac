...
describe('SqliteConteoRepository', () => {
  ...
  describe('lecturas de la sesión (lista de la pantalla de conteo)', () => {
    /* Ids de las lecturas de un detalle, en orden de registro. */
    async function lecturaIds(detalleId: number): Promise<number[]> {
      const r = await db.query(
        `SELECT id FROM sod_conteo_lectura WHERE detalle_id = ? ORDER BY id`,
        [detalleId]
      );
    ...
    it('adjustLectura: +delta sube esa lectura y su detalle, sin tocar las otras', async () => {
      const item = await scan(12);
      await scan(5);
      const [l1] = await lecturaIds(item.id);
    ...
    it('adjustLectura: -delta con piso en 0, y la fila NO se borra', async () => {
      const item = await scan(1);
      await scan(4);
      const [l1] = await lecturaIds(item.id);

      await repo.adjustLectura(l1, -5); // pide -5 sobre 1: mueve -1
    ...
    it('adjustLectura: rechaza una lectura cuyo detalle ya no está EN_CURSO', async () => {
      const item = await scan(3);
      const [l1] = await lecturaIds(item.id);
      await repo.cerrarTag(CONTEO_ID, UBICACION_ID, OPERADOR_ID);

      await expectAsync(repo.adjustLectura(l1, 1)).toBeRejectedWithError(/no se puede modificar/);
    });

    it('deleteLectura: borra solo esa fila y descuenta su cantidad del detalle', async () => {
      const item = await scan(12);
      await scan(5);
      const [l1, l2] = await lecturaIds(item.id);

      await repo.deleteLectura(l1);

      expect(await lecturaIds(item.id)).toEqual([l2]);
      expect(await cantidadFisica()).toBe(5);
      expect(await sumaLecturas(item.id)).toBe(5);
    });

    it('deleteLectura: al irse la última lectura, el detalle se borra también', async () => {
      const item = await scan(9);
      const [l1] = await lecturaIds(item.id);
    ...
    it('deleteLectura: rechaza una lectura cuyo detalle ya no está EN_CURSO', async () => {
      const item = await scan(3);
      const [l1] = await lecturaIds(item.id);
      await repo.cerrarTag(CONTEO_ID, UBICACION_ID, OPERADOR_ID);

      await expectAsync(repo.deleteLectura(l1)).toBeRejectedWithError(/no se puede eliminar/);
    });
  });
